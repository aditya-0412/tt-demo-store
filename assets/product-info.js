if (!customElements.get("product-info")) {
  customElements.define(
    ...
    class ProductInfo extends HTMLElement {
      ...
      // Custom Code Start: Variant based description update
      updateVariantDescription(html) {
        // For variant description tab layout
        const oldElementTab = this.querySelector("#variant-desc");
        const newElementTab = html.querySelector("#variant-desc");
        if (oldElementTab && newElementTab) {
          HTMLUpdateUtility.viewTransition(oldElementTab, newElementTab);
        }

        // For variant description collapsible row layout
        const oldElementCollapsible = this.querySelector("#variant-desc-row");
        const newElementCollapsible = html.querySelector("#variant-desc-row");
         if (oldElementCollapsible && newElementCollapsible) {
            HTMLUpdateUtility.viewTransition(oldElementCollapsible, newElementCollapsible);
          }
      }
      // Custom Code End: Variant based description update

      handleUpdateProductInfo(productUrl) {
        return (html) => {
          const variant = this.getSelectedVariant(html);

          this.updateVariantDescription(html); // Custom Code: Function Call
